...
def calculate_demographic_data(print_data=True):
    ...
    country_earning = df[df['salary'] == '>50K'].groupby('native-country').size()
    country_total = df.groupby('native-country').size()
    country_percentage = (country_earning / country_total).sort_values(ascending=False)
    highest_earning_country = country_percentage.index[0]
    highest_earning_country_percentage = round(country_percentage.iloc[0] * 100, 1)
